...
import { WritingData } from '../data/writings';
import { SectionConfig, AccentColor, SocialLinks } from '../App';
...
interface AdminPageProps {
  ...
  onUpdateWritings: (writings: WritingData[]) => void;
  mainHeader: string;
...
interface Writing extends WritingData {
  title: string; 
  content: string;
  mood?: string;
  date?: string;
...
const AdminPage: React.FC<AdminPageProps> = ({ writings, onUpdateWritings, mainHeader, setMainHeader, sections, setSections, iconMap, socialLinks, setSocialLinks }) => {
  ...
  // Local states for managing data within the admin page
  ...
  // formData is for new entries, separate from editingWriting to avoid issues with controlled/uncontrolled inputs.
  const [formData, setFormData] = useState<Partial<Writing>>({ title: '', content: ''});
  const [showAddEditModal, setShowAddEditModal] = useState(false);

  const [searchTerm, setSearchTerm] = useState('');
  const [filterSection, setFilterSection] = useState('all');

  // States for site header and section management
  ...
  // Initialize sectionFormData with default values including iconName
  const [sectionFormData, setSectionFormData] = useState<Partial<SectionConfig>>({ title: '', accent: 'emerald', iconName: 'BookOpen'});
  const [showSectionModal, setShowSectionModal] = useState(false);

  // Add local state for social links, synced with prop
  const [currentSocialLinks, setCurrentSocialLinks] = useState<SocialLinks>(socialLinks);
  const [currentMainHeaderLocal, setCurrentMainHeaderLocal] = useState(mainHeader); // Local state for header input
  ...
  useEffect(() => {
    setCurrentMainHeaderLocal(mainHeader);
  }, [mainHeader]);
  ...
  const handleOpenAddModal = () => {
    setEditingWriting(null); // Not editing an existing one
    setFormData({ title: '', content: '', sectionId: sections[0]?.id || undefined }); // Reset form for new entry
    setShowAddEditModal(true);
  };

  const handleOpenEditModal = (writing: WritingData) => {
    setEditingWriting(writing as Writing); // Cast to Writing
    setFormData(writing as Writing); // Populate form with existing data
    setShowAddEditModal(true);
  };
  
  const handleDeleteWriting = (id: string) => {
    if (window.confirm('Are you sure you want to delete this writing?')) {
      const updatedWritings = currentWritings.filter(w => w.id !== id);
      onUpdateWritings(updatedWritings); // Update in App.tsx
    }
  };
  ...
  const handleSaveWriting = () => {
    ...
    if (editingWriting && editingWriting.id) {
        ...
        updatedWritings = currentWritings.map(w => 
            w.id === editingWriting.id ? { ...w, ...formData, id: editingWriting.id } : w
        );
    } else {
        // Adding new writing: ensure all fields from WritingData are present
        const newId = formData.id || `writing-${Date.now()}`; // Ensure an ID
        const newWriting: WritingData = {
            ...formData, // Spread formData
            id: newId,
            title: formData.title!, 
            content: formData.content!,
            likes: formData.likes || 0, // Default likes to 0
            date: formData.date || new Date().toISOString().split('T')[0], // Default date
            // sectionId, mood are already in formData or undefined
        };
        updatedWritings = [...currentWritings, newWriting];
    }
    onUpdateWritings(updatedWritings);
    setShowAddEditModal(false);
    setEditingWriting(null); // Clear editing state
  ...
  const handleOpenEditSectionModal = (section: SectionConfig) => {
    ...
    setSectionFormData({ ...section }); // Populate form with existing section data
    setShowSectionModal(true);
  };

  const handleDeleteSection = (id: string) => {
    if (window.confirm('Are you sure you want to delete this section? This will not delete poems but will unassign them.')) {
      const updatedSections = currentSections.filter(s => s.id !== id);
      // Unassign poems from the deleted section
      const updatedWritings = currentWritings.map(w => w.sectionId === id ? { ...w, sectionId: undefined } : w);
      setSections(updatedSections); // Update sections in App.tsx
      onUpdateWritings(updatedWritings); // Update writings in App.tsx
    }
  };
  ...
  const handleSaveSection = () => {
    ...
    let updatedSections;
    const iconComponent = iconMap[sectionFormData.iconName!] || DefaultSectionIcon;
    ...
    if (editingSection && editingSection.id) {
        ...
        // Adding new section
        const newId = sectionFormData.id || `section-${Date.now()}`;
        const newSection: SectionConfig = {
            ...
            icon: iconComponent,
            // Provide defaults for other SectionConfig fields
            iconClass: `text-${sectionFormData.accent}-400`, // Example default
            navClass: `hover:text-${sectionFormData.accent}-400`, // Example default
            bgClass: `bg-${sectionFormData.accent}-400`, // Example default
            writingIds: [], // New sections start with no writings associated directly via writingIds
        };
  ...
  const handleSectionOrderChange = (index: number, direction: 'up' | 'down') => {
    ...
    setSections(newSections); // Update in App.tsx
  };
  ...
  const handleSaveSocialLinks = () => {
    ...
    alert('Social links saved!'); // Basic feedback
  };

  const handleSaveMainHeader = () => {
    setMainHeader(currentMainHeaderLocal);
          ...
          {activeTab === 'content' && (
            ...
            >
              ...
              <div className="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg mb-8">
                ...
                />
                <button 
                  onClick={() => setMainHeader(currentMainHeader)}
                        ...
                        <table className="min-w-full divide-y divide-gray-700">
                            ...
                            <tbody className="bg-gray-800 divide-y divide-gray-700">
                                {filteredWritings.map(writing => {
                                    ...
                                    return (
                                        <tr key={writing.id}>
                                            <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium text-gray-100 max-w-[100px] sm:max-w-xs truncate" title={writing.title}>{writing.title || 'Untitled'}</td>
          ...
          {activeTab === 'analytics' && (
            ...
            >
                ...
                <div className="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg">
                    ...
                    <p className="text-base sm:text-lg font-semibold truncate" title={mostLikedWriting?.title}>{mostLikedWriting?.title || 'N/A'}</p>
                    {mostLikedWriting && <p className="text-xs sm:text-sm text-gray-400">{mostLikedWriting.likes} likes</p>}
                </div>
            </motion.div>
          )}
          ...
          {activeTab === 'settings' && (
            ...
            >
              ...
              <div className="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg">
                ...
                <input 
                  type="text"
                  value={currentMainHeaderLocal}
                  onChange={(e) => setCurrentMainHeaderLocal(e.target.value)}
